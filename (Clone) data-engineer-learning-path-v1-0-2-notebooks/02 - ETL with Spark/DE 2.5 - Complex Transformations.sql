...
create temp view raw_temp as select string(key),string(value) FROM events_raw;

-- COMMAND ----------

create temp view cln_temp as 
select key,from_json(value,schema_of_json('{"device":"macOS","ecommerce":{},"event_name":"checkout","event_previous_timestamp":1593880801027797,"event_timestamp":1593880822506642,"geo":{"city":"Traverse City","state":"MI"},"items":[{"item_id":"M_STAN_T","item_name":"Standard Twin Mattress","item_revenue_in_usd":595.0,"price_in_usd":595.0,"quantity":1}],"traffic_source":"google","user_first_touch_timestamp":1593879413256859,"user_id":"UA000000107384208"}') ) as val_struct from raw_temp;

-- COMMAND ----------

create temp view more_clned_vw as 
with cte as (select val_struct.geo.*,explode(val_struct.items) as new_items from cln_temp)
select *,new_items.* from cte


-- COMMAND ----------

select * from more_clned_vw
...
SELECT collect_set(event_name),array_distinct(flatten(collect_set(items.item_id))) FROM exploded_events
...
FROM item_purchases
PIVOT (
  sum(purchase_revenue_in_usd) FOR item_id IN (
    'P_FOAM_K',
    'M_STAN_Q',
    'P_FOAM_S',
    'M_PREM_Q',
